...
**Second, more serious deviation found and fixed during the session's final verification pass**: `tsc`, `eslint`, and `vitest` all passed cleanly, but `npm run build` failed with `Server Actions must be async functions` for all five exported helpers. `test-cases.ts` has `"use server"` at the top, and Next.js requires every export from such a file to be async (every export is treated as a potential serialized Server Action reference) — this plan's own Step 1 instruction ("change each declaration from `function name(...)` to `export function name(...)`") never accounted for that, since these five are plain synchronous utilities. Fixed by moving all five into a new plain module, `src/lib/actions/test-cases-helpers.ts` (no directive) — `test-cases.ts` now imports them from there instead of declaring them locally, and the test file imports directly from the new module too (re-exporting them from `test-cases.ts` would hit the same constraint again). This is the exact risk already flagged during the Weekly Report plan's design, where `computeWeeklyReportMetrics` was deliberately kept out of its `"use server"` file for this reason — worth remembering for any future plan that exports a helper from an actions file: run the real `npm run build`, not just `tsc`/`eslint`, since only the build enforces this. Committed separately (`4c006be`).
